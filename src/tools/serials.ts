// Serial Number tools for inFlow MCP Server
//
// Two approaches for serial number retrieval:
// 1. Order-based: Use `include=lines` on sales/purchase orders to get serials assigned to orders
// 2. Product-based: Use `include=inventoryLines` on products to get all serials for a product
//
...
export function registerSerialTools(server: McpServer, client: InflowClient): void {
  // Get serials from Sales Order
  server.tool(
    'get_sales_order_serials',
    'Extract serial numbers from a specific sales order. Returns all serials assigned to line items on this order.',
    {
  ...
  // Get serials from Purchase Order
  server.tool(
    'get_purchase_order_serials',
    'Extract serial numbers from a specific purchase order. Returns all serials assigned to line items on this order.',
    {
  ...
  // Search serial across orders
  server.tool(
    'search_serial_number',
    'Search for a serial number across fulfilled sales orders. Returns the order details if found. WARNING: This searches by fetching orders - may be slow for large order volumes.',
    {
            ...
            text: JSON.stringify(
              {
                ...
                ordersSearched,
                message:
                  'Serial number not found in fulfilled sales orders. Check your authoritative database for serial number data.',
              },
              null,
              2
            ),
  ...
  // List all serials from fulfilled orders
  server.tool(
    'list_serial_numbers',
    'List all serial numbers from fulfilled sales orders. Useful for building a serial number inventory. WARNING: This fetches all orders and may be slow.',
    {
      maxOrders: z
        .number()
        .optional()
  ...
  server.tool(
    'get_product_serials',
    'Get all serial numbers for a specific product using inventoryLines. Much faster than order-based lookup. Returns serials with stock status (quantityOnHand=0 means sold/shipped).',
    {
  ...
  server.tool(
    'list_all_serials',
    'List all serial numbers across ALL products that track serials. Uses inventoryLines for fast retrieval. Much faster than order-based aggregation.',
    {
      maxProducts: z
        .number()
        .optional()
