...
export function registerProductTools(server: McpServer, client: InflowClient): void {
  ...
  server.tool(
    ...
    'Search and list products from inFlow Inventory with optional filtering',
    {
      ...
      trackSerials: z.boolean().optional().describe('Filter to only return products that track serial numbers'),
  ...
  server.tool(
    ...
    'Get detailed information about a specific product by ID. Use include=itemBoms to get the bill of materials (components needed to manufacture this product).',
    {
      ...
      include: z
        .array(z.string())
        .optional()
        .describe('Related data to include. Options: category, customFields, itemBoms (bill of materials), inventoryLines (serial numbers for trackSerials products)'),
    },
    async (args) => {
